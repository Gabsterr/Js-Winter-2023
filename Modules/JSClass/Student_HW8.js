...
//Homework 8
    ...
    changeName (newName) {
    if (newName === studentInfo.sName) {
        console.log('\n\n Name cannot be updated; as you have same name already stored in the system.');
    } else { 
        newName = studentInfo.sName;
        console.log('\n\nYour name in the system is updated to new-name ')
    }

}
            
    changeCourse (newCourseName)
    if (!isCourseValid(newCourseName) || newCourseName != courseData)  {
         console.log('\n\nInstitute does not provide any course on new-course-name');
        } else if (newCourseName === studentInfo.sCourseName) {
            console.log('\n\nYou are already enrolled in the course');
        } else {
                newCourseName = studentInfo.sCourseName;
                courseObject
        }
